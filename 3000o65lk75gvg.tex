...
\end{equation}
とする（正確には、$\bigotimes_p \mathcal{H}_p$の元のほとんどは$\mathcal{H}$には含まれないため、分割とは異なるが、簡単のために分割の表現を用いる。フォック空間も、すべてが物理的にありえる状態ではなく、対象又反対象な元のみが物理的な状態であるので、それと同様に$\bigotimes_p \mathcal{H}_p$の元のうち一部のみがぶ）。\\
