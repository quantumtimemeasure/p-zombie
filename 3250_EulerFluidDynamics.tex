...
\begin{equation}
    ...
    \bigotimes_p \ket{\rho_p(t),\pi_p(t)},\bm{v}_p(t) \in \bigotimes_p \mathcal{H}_p^a
\end{equation}
とかけるとする。ボリューム間での物質の流出入が発生する。ここで、$\rho_p(t)$はボリューム$V(t)_p$の密度（量子力学における密度行列、密度演算子ではなく、古典流体力学における密度である）、$\pi_p(t)$はボリューム$V(t)_p$の圧力である。$\bm{v}_p(t)$は群速度である。
当然ながら、
\begin{equation}
